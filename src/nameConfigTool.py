...
# PySide6 라이브러리 임포트 (PySide2에서 변경)
...
# JalLib 모듈 임포트
from pyjallib import NamingConfig
from pyjallib import NamePart, NamePartType
...
class NameConfigToolUI(QMainWindow):
    ...
    def loadDefaultConfig(self):
        ...
        try:
            # 기본 설정 로드 - 경로 지정
            self.configObj = NamingConfig()
            config_path = os.path.join("src", "ConfigFiles", "namingConfig.json")
            success = self.configObj.load(config_path)
            
            if success:
                self.updateUI()
                self.filePathLabel.setText(f"파일: {os.path.basename(config_path)}")
            else:
                QMessageBox.warning(self, "경고", f"기본 설정 파일({config_path})을 로드할 수 없습니다.")
